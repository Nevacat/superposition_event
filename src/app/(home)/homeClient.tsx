...
import { useVisitedStore } from '@/store/visit';
...
const HomeClient = () => {
  ...
  // 전역 변수에 저장되어있는 방문 여부를 확인하여 false라면 true로 변경
  const { setVisited, visited } = useVisitedStore();
  ...
  return (
    <div className='w-full h-full  max-w-screen-sm bg-black overflow-y-scroll overflow-x-hidden '>
      ...
      <div className='text-white text-[32px] font-bold text-center translate-y-[-130px] flex flex-col gap-2 animate-fade-in px-2'>
        <p>
      ...
      <div className='min-h-[400px] relative flex flex-col items-center justify-center gap-6 px-4'>
        ...
        <div className='flex items-center'>
          ...
          />
          <button
            className='bg-[#6e28ac] opacity-80 w-[60px] h-12 rounded-r-full flex items-center justify-center'
            onClick={onSubmit}
          >
      ...
      {confirms.status && <ConfirmModal confirms={confirms} setConfirms={setConfirms} />}
    </div>
  );
};
